...
public class ImageOrderAndZBufferStrategyTest {
	...
	public void setUp() throws Exception {
		byte[][] colorAndAlphaBuffersData = new byte[IMAGES_PATH.length][];
		
		int i = 0;
		for (String testImageFilePath : IMAGES_PATH) {
			colorAndAlphaBuffersData[i++] = ImageUtil.readImageDataAsByteArray(testImageFilePath);
		}
		
		float[][] depthBuffersData = new float[IMAGES_PATH.length][];
		i = 0;
		for (String testImageFilePath : IMAGES_PATH) {
			float value = (3 - i) * (0.5f / (float) IMAGES_PATH.length);
	...
	public void testCompose() throws IOException {
		...
		int[] expectedPixelBufferData = ImageUtil.readImageDataAsIntArray(COMPOSITION_RESULT_FILE_PATH);
